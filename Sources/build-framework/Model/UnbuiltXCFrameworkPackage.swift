...
struct UnbuiltXCFrameworkPackage {
	...
	func buildXCFrameworkPackage(openldapVersion: String) throws {
	...
	func packageFile(forVersion version: String?, checksums: [String: String?]) -> String {
		...
		packageString.append(types.map{ type in
			...
			if let checksum = checksum {
				/* TODO: Customize URL */
				return #"\#t\#t.binaryTarget(name: "\#(buildPaths.productName)-\#(type)", url: "https://github.com/xcode-actions/\#(buildPaths.productName)/releases/download/\#(BuiltTarget.normalizedOpenLDAPVersion(version!))/\#(buildPaths.productName)-\#(type).xcframework.zip", checksum: "\#(checksum)")"#
			} else {
