...
struct XCFrameworkDependencySource {
	...
	func downloadAndExtract(in destinationFolder: FilePath) async throws -> XCFrameworkDependency {
		...
		guard !url.isFileURL || url.path != destPath.string else {
			...
			return try XCFrameworkDependency(path: destPath)
		}
